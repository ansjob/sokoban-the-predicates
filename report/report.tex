...
By doing that we could see that the memory filled up after certain amount of time. 
Then the agent is using the CPU until the memory is filled up and after that the garbage collector tries to free 
memory but is unable to do so because almost every object is referenced by the agent. 
...
This is of course a bad usage of the time frame. 
...
find a solution for puzzles where there are a lot of tunnels. 
If this also fails within a time limit the agent will start to randomize 
the weights and try a lot of different weights for short periods of time. 

The problem with this solution is that all previous work becomes useless since the agent starts over again. 
But because of the memory problem it would not have done any useful work anyway so 
it is best to start over again and hope that the new approach is better. 
...
When sampling the agent we noticed that a lot of time was spent in the Sokoban states \verb!equals()! and \verb!hashCode()! methods. 
...
a coordinate's neighbours, which is very unneccessary since
...
Another is to implement so called ''tunnel macros'',
which would recognize tunnels, and prohibit the player
from pushing a box part way through a tunnel and consider this as a new state.
